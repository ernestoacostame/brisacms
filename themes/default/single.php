...
<div class="page-header">
  <h1 class="page-title"><?= htmlspecialchars($post['title'] ?? 'Sin título') ?></h1>
...
<article>
  <header class="article-header">
    <div class="article-meta">
      ...
      <a href="<?= $base ?>/category/<?= urlencode($cat) ?>" class="post-category"><?= htmlspecialchars($cat) ?></a>
      <?php endforeach; ?>
    </div>
    <h1 class="article-title"><?= htmlspecialchars($post['title'] ?? 'Sin título') ?></h1>
    <?php if (!empty($post['excerpt'])): ?>
    <p style="color:var(--muted);font-size:1.05rem;margin-top:0.75rem;max-width:var(--max-w);font-family:var(--font-body)"><?= htmlspecialchars($post['excerpt']) ?></p>
    <?php endif; ?>
  </header>

  <?php if (!empty($post['featured_image'])): ?>
  <img src="<?= htmlspecialchars($post['featured_image']) ?>" alt="<?= htmlspecialchars($post['title']) ?>" class="article-featured">
  <?php endif; ?>
<?php endif; ?>
...
<div class="tags">
  ...
  <a href="<?= $base ?>/tag/<?= urlencode($tag) ?>" class="tag">#<?= htmlspecialchars($tag) ?></a>
  <?php endforeach; ?>
</div>
<?php endif; ?>
...
<div style="margin-top:3rem;padding-top:2rem;border-top:1px solid var(--border)">
  <a href="<?= $base ?>/" style="color:var(--muted);text-decoration:none;font-size:0.9rem;display:inline-flex;align-items:center;gap:0.35rem">
    ← <?= $is_page ? 'Volver al inicio' : 'Volver a los artículos' ?>
  </a>
</div>
