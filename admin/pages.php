...
?>
  ...
  <div class="page-body">
    ...
    <div class="card">
      <div class="card-header"><span class="card-title"><?= $result['total'] ?> Pages</span></div>
      <?php if (empty($result['items'])): ?>
        <div class="card-body" style="text-align:center;color:var(--muted);padding:3rem">
          No pages yet. <a href="<?= base_url() ?>/admin/editor.php?type=pages" style="color:var(--accent)">Create your first page →</a>
        </div>
      <?php else: ?>
      <table class="table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <a href="<?= base_url() ?>/admin/editor.php?type=pages&slug=<?= htmlspecialchars($post['slug']) ?>" class="btn btn-secondary btn-sm">Edit</a>
              <?php if ($post['status'] === 'published'): ?>
              <a href="<?= base_url() ?>/page/<?= htmlspecialchars($post['slug']) ?>" target="_blank" class="btn btn-secondary btn-sm">View</a>
              <?php endif; ?>
            </td>
          </tr>
          <?php endforeach; ?>
